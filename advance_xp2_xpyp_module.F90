...
module advance_xp2_xpyp_module
  ...
  !=============================================================================
  subroutine advance_xp2_xpyp( tau_zm, wm_zm, rtm, wtrc_rtm, wprtp, &
                               wtrc_wprtp, thlm, &
                               wpthlp, wpthvp, um, vm, wp2, wp2_zt, &
                               wp3, upwp, vpwp, sigma_sqd_w, Skw_zm, &
                               Kh_zt, rtp2_forcing, wtrc_rtp2_forcing, &
                               thlp2_forcing, &
                               ...
                               l_iter, dt, &
                               sclrm, wpsclrp, & 
                               rtp2, wtrc_rtp2, thlp2, rtpthlp, &
                               wtrc_rtpthlp, up2, vp2,  & 
                               err_code, & 
   ...
   real( kind = core_rknd ), intent(in), dimension(gr%nz,wtrc_nwset) :: &
    ...
    do m=1,wtrc_nwset
      ...
      call xp2_xpyp_rhs( xp2_xpyp_sclrprtp, dt, l_iter, a1, a1_zt, &  ! Intent(in)
                         wp2_zt, wtrc_wprtp(:,m), &               ! Intent(in)
                         wtrc_wprtp_zt(:,m), wp3_on_wp2, &        ! Intent(in)
                         wp3_on_wp2_zt, wprtp, wprtp_zt, &        ! Intent(in)
                         wtrc_rtm(:,m), wtrc_rtm(:,m), &          ! Intent(in)
                         wtrc_rtp2(:,m), wtrc_rtp2_forcing(:,m),& ! Intent(in)
                         rho_ds_zm, rho_ds_zt, invrs_rho_ds_zm, & ! Intent(in)
                         C2rt_1d, tau_zm, rt_tol**2, beta, &      ! Intent(in)
